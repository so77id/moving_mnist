        ...
        with open(list_filename, 'w') as writer:
            for combination, video_list in dataset.items():
                ...
                for i, video in enumerate(video_list):
                    video_path = "%s/%05d" % (comb_path, i)

                    writer.write("{} {}\n".format(video_path, comb_name))
                    create_dirs([video_path])
                    for j, image in enumerate(video):
                        output_filename = "%s/%04d.jpg" % (video_path, j)
...
def main():
    ...
    dataset_path = config.dataset.original.dataset_path
    train_list_filename = config.dataset.original.train_list
    test_list_filename = config.dataset.original.test_list
    filetype = config.dataset.parameters.filetype
    width = config.dataset.parameters.width
    height = config.dataset.parameters.height
    seq_len = config.dataset.parameters.seq_len
    seqs_per_class = config.dataset.parameters.seqs_per_class
    seqs_per_class_test = config.dataset.parameters.seqs_per_class_test
    num_sz = config.dataset.parameters.num_sz
    nums_per_image = config.dataset.parameters.nums_per_image
